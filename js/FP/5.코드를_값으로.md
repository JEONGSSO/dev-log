# 코드를 값으로 다루어 표현력 높이기

## go

```js

const add = () => a + b;
const reduce2 = (f, iter, acc) => { 
   if (acc === undefined) { // acc 초기값을 넘기지 않으면 iter에서 맨 앞에 값이 초기값으로 들어감. 
      iter = iter[Symbol.iterator]();
      acc = iter.next().value;
   }

   for (const v of iter) {
      acc = f(acc, v);
   }
   return acc;
};

const go = (...args) => console.log(args) // [0, function (), function (), function (), function ()]

//go 함수를 사용하면 특정 리스트를 축약해나가는 코드 작성이 쉬워진다
const go = (...args) => reduce2((a, f) => f(a), args)

go(
   0, // 0이 바로 밑 함수인자 a로 들어감
   a => a + 1, // 1이 바로 밑 함수인자 a로 들어감...
   a => a + 10,
   a => a + 100,
   console.log
)
// 111
---
```
## pipe

```js

// pipe 함수는 인자들을 연속적으로 실행하여 하나의 함수를 만드는것
const pipe = (...fns) => (a) => go(a, ...fns)

const pipeFn = pipe(
   a => a + 1,  // fns로 들어간다.
   a => a + 10,
   a => a + 100,
   console.log
)

console.log(pipeFn(0)) // pipe두 번째인자인 a로 들어간다.

```

---
``` js
   go(
      add(0, 1), // 요렇게 사용도 가능.
      a => a + 10,
      a => a + 100,
      console.log
   )

   const pipe = (fn, ...fns) => (...as) => go(fn(as), ...fns)
   const pipeFn = pipe(
      (a, b) => a + b,
      a => a + 10,
      a => a + 100,
      console.log
   )
   
   console.log(pipeFn(add(0, 1))) // pipe는 go(add(0, 1) ...) 처럼 사용하려면 이렇게 사용해야 하기때문에 조금 바꿔주어야 한다.

   const pipe = (fn, ...fns) => (...as) => go(fn(...as), ...fns) // add(0, 1) 1이 fns의 인자로 ~
   const pipeFn = pipe(
      add, //pipe의 fn으로 들어가고
      a => a + 10, // fns
      a => a + 100 // fns
   )
   console.log(pipeFn(0, 1)) // ...as에 들어간다
```

---
4.map_filter_reduce에 go함수 사용해서 읽기 좋은 코드 만들기

```js
   fp.go(
      fp.map(v => v.price, products),
      products => fp.filter(price => price > 20000, products),
      products => fp.reduce2(add, products),
      console.log
   )
```

---
## curry

```js
   const add = (a, b) => a + b;
   const curry = f => 
   (a, ..._) => _.length ? f(a, ..._) : (..._) => f(a, ..._); 
// 맨 처음인자로 함수를 받고 함수를 리턴 두 번째 인자가 들어오면 세번째 인자가 있는지 확인 후 없으면 함수 리턴 있으면 함수 실행 세 번째 인자가 들어오면 함수실행

console.log(curry(add)(1)(1)) //2
const add1 = curry(add)(1)
console.log(add1(1)) //2                                     

const mult = curry((a, b) => a * b);
console.log(mult)    // (a, ..._) => (_.length ? f(a, ..._) : (..._) => f(a, ..._))
console.log(mult(1)) // (..._) => f(a, ..._)

// map filter reduce curry적용
const map = curry((f, iter) => {
  const arr = [];
  for (const v of iter) {
    arr.push(f(v));
  }
  return arr;
});

const filter = curry((f, iter) => {
  const arr = [];
  for (const v of iter) {
    if (f(v)) arr.push(v);
  }
  return arr;
});

const reduce2 = curry((f, iter, acc) => {
  if (acc === undefined) {
    iter = iter[Symbol.iterator]();
    acc = iter.next().value;
  }

  for (const v of iter) {
    acc = f(acc, v);
  }
  return acc;
});

// 잎에서 사용한 go함수 예제 보기좋게 바꾸기
fp.go(
  dummy.products,
  products => fp.map(v => v.price)(products), // 요렇게 나중에 판단하게 할수 있다.
  products => fp.filter(price => price > 20000)(products),
  products => fp.reduce2(add)(products),
  log
);

// 이게 뭐가 간단하냐 ?

// products를 받아서 그대로 전달한다는 이야기는
fp.go(
  dummy.products,
  products => fp.map(v => v.price)(products), // products를 받아서 map에 그대로 전달한다는 이야기는 이 자리에 들어오는 함수(map)가 products를 받는다는 이야기
  products => fp.filter(price => price > 20000)(products),
  products => fp.reduce2(add)(products),
  log
);


fp.go(
  dummy.products,
   fp.map(v => v.price),
   fp.filter(price => price > 20000),
   fp.reduce2(add),
   log
);
//나는 products 를 지워도 되는게 잘 이해가 가지않았다.

// 인동님 힌트1: a => f(a) 라는 함수는 그냥 f와 하는일이 같다

// 인동님 힌트2: 아래 3개의 함수에 3을 넣으면 모두 4가 나온다는 설명이다.
const add1 = a => a + 1;
const f1 = a => add1(a);
const f2 = add1;

//go에 args 콘솔로 찍어보면 ...
(3) [Array(5), ƒ, ƒ]
0: (5) [{…}, {…}, {…}, {…}, {…}]
1: ƒ (products)
2: ƒ ()
```
