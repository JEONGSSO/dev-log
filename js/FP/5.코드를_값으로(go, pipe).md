...
const go = (...args) => reduce2((acc, f) => console.log(args), args);
//go에 args 콘솔로 찍어보면 ...
(3) [Array(5), ƒ, ƒ]
0: (5) [{…}, {…}, {…}, {…}, {…}]
...
const reduce2 = (f, iter, acc) => {
  ...
  return acc; // 최초에 acc를 넘기는데 args 맨 처음 값인 Array (5) [{…}, {…}, {…}, {…}, {…}] 리턴
};

const go = (...args) => reduce2((acc, f) => f(acc), args);
// go함수에서 reduce 인자 부분에 acc는  [{…}, {…}, {…}, {…}, {…}] 값이 들어오고 f 부분엔 그 다음 부분인 map(v => v.price)을
...
// n만원 미만(초과) 값을 더한다고 했을 때 map과 reduce2부분은 항상 같은 부분을 부르게 되어 중복이 될것이다.
